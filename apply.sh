...
original_author="devanshukla"
original_name="new_one_template"
original_urlname="new_one_template"
original_description="Awesome new_one_template created by devanshukla" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/new_one_template "${name}"
...
then
    ...
    echo "then run 'new_one_template' or 'python -m new_one_template'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'new_one_template' or 'python -m new_one_template' or 'poetry run new_one_template'"
fi
