...
- 🎯 Entry points to execute your program using `python -m <new_one_template>` or `$ new_one_template` with basic CLI argument parsing.
...
---
# new_one_template Flask Application

Awesome new_one_template created by devanshukla
...
```bash
git clone https://github.com/devanshukla/new_one_template new_one_template
cd new_one_template
make install
```
...
```bash
new_one_template create-db   # run once
new_one_template populate-db  # run once (optional)
new_one_template add-user -u admin -p 1234  # ads a user
new_one_template run
```
